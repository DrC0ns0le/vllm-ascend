    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if request.num_computed_tokens == 0:
                    ...
                    if self.connector is not None:
                        ...
                        if hit_diverged and num_external_computed_tokens == 0:
                            # Without an external suffix, reconcile divergent
                            # hybrid hits to a boundary shared by every group.
                            (
                                new_computed_blocks,
                                num_new_local_computed_tokens,
                                request.shared_prefix_boundary,
                ...
                # Record only after allocation succeeds: lookups for requests
                # that were not admitted must not affect prefix-cache metrics.
